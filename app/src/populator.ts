...
interface Populator {
    generatePeople(base: UserDB): void;
}


class PostgresPopulator implements Populator {
    ...
    async generatePeople(base: UserDB): Promise<void> {
        ...
        for (let i = 0; i < 100; i++) {
            base.addNewUser({
                ...
                surname: lastnames[Math.floor(Math.random() * 300)],
                email: `${lastnames[Math.floor(Math.random() * 50)]}@${firstNames[Math.floor(Math.random() * 321)]}.pl`,
                password: await bcrypt.genSalt(1),
                password_repeat: "haha"
            });
        }
